...
const QUESTION_WORDS = new Set([
  "zer", "nor", "nork", "nori", "non", "nora", "nondik", "nola", "zenbat", "zein", "zergatik", "nongo", "nongoa", "zertaz"
]);

function hasOvertNounSubject(words) {
  for (let i = 0; i < words.length; i++) {
    const cw = cleanWord(words[i]);
    if (QUESTION_WORDS.has(cw)) {
      return true;
    }
    ...
    // Any noun with definite article -a / -ak / -aren / -ari / -an / -tik / -ra
    if (/(a|ak|aren|en|ari|ei|an|etan|tik|etatik|ra|etara|ekin|rekin|ko|koa|koak)$/.test(cw) && !ALL_PRONOUNS.has(cw) && !COPULAS.includes(cw)) {
      return true;
    }
  }
  return false;
...
function generateSingleClauseVariations(clause, prompt = "") {
  ...
  if (ezIdx !== -1 && ezIdx + 1 < cleanWords.length) {
    ...
    const isAux = isFiniteVerb(nextWord);
    ...
    if (isAux) {
      ...
      if (rest.length >= 2) {
        ...
        if (subjBefore) {
          ...
          variations.add(`ez ${aux} ${part2} ${part1} ${subjBefore.toLowerCase()}`);
          variations.add(`ez ${aux} ${part1} ${part2} ${subjBefore.toLowerCase()}`);
          if (ALL_PRONOUNS.has(cleanWord(subjBefore))) {
          ...
          variations.add(`ez ${aux} ${part2} ${part1}`);
          variations.add(`ez ${aux} ${part1} ${part2}`);
          const p = ERGATIVE_PRONOUNS[nextWord] || SUBJECT_PRONOUNS[nextWord];
          if (p) {
            variations.add(`${p} ez ${aux} ${part2} ${part1}`);
            variations.add(`${p} ez ${aux} ${part1} ${part2}`);
            variations.add(`ez ${aux} ${part2} ${part1} ${p}`);
            variations.add(`ez ${aux} ${part1} ${part2} ${p}`);
          }
        }
      } else if (rest.length === 1) {
        const pred = rest[0];
        if (subjBefore) {
          variations.add(`${subjBefore} ez ${aux} ${pred}`);
          variations.add(`ez ${aux} ${pred} ${subjBefore.toLowerCase()}`);
          if (ALL_PRONOUNS.has(cleanWord(subjBefore))) {
            variations.add(`ez ${aux} ${pred}`);
          }
        } else {
          variations.add(`ez ${aux} ${pred}`);
          const p = ERGATIVE_PRONOUNS[nextWord] || SUBJECT_PRONOUNS[nextWord];
          if (p) {
            variations.add(`${p} ez ${aux} ${pred}`);
            variations.add(`ez ${aux} ${pred} ${p}`);
          }
        }
      }
      ...
      if (partitiveIdx !== -1) {
        ...
        const sub = generateSingleClauseVariations(withDef, prompt);
...
console.log(`Saved updated challenges.json successfully!`);
