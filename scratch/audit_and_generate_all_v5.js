...
const processedChallenges = challenges.map(c => {
  ...
  if (c.type === 'TRANSLATE') {
    ...
    if (isBasque) {
      ...
      // Also generate variations on existing accepted answers
      existingAnswers.forEach(ea => {
        const fromExisting = generateBasqueVariations(ea, updated.prompt || '');
...
const sampleIds = [21, 30, 31, 34, 71, 74, 76, 77, 79, 81, 84, 91, 94, 404, 420, 443, 447, 507, 713, 724, 780, 1721];
