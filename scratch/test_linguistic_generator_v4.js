...
function generateBasqueVariations(originalTarget, prompt = "") {
  ...
  if (introMatch) {
    ...
    if (candidateIntro.toLowerCase() === "ez") {
      // If candidateIntro is "Ez", only treat as intro if firstRestWord is NOT a finite verb
      // OR if prompt starts with "No,"
...
function generateSingleClauseVariations(clause, prompt = "") {
  ...
  if (ezIdx !== -1 && ezIdx + 1 < cleanWords.length) {
    ...
    if (isAux) {
      ...
      // Participle and object inversion in negative sentences
      if (rest.length >= 2) {
        ...
        if (subjBefore) {
          ...
          // Pro-drop if subject before is pronoun
          ...
          variations.add(`ez ${aux} ${part2} ${part1}`);
          variations.add(`ez ${aux} ${part1} ${part2}`);
          // Add compatible subject pronoun
