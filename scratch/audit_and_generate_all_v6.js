...
// Synonyms mapping
const SYNONYMS = [
  [/\bcamareroa\b/gi, "zerbitzaria"],
  [/\bzerbitzaria\b/gi, "camareroa"],
  [/\bcamarero bat\b/gi, "zerbitzari bat"],
  [/\bzerbitzari bat\b/gi, "camarero bat"],
  [/\bcamarero\b/gi, "zerbitzari"],
  [/\bzerbitzari\b/gi, "camarero"],
  [/\bkotxea\b/gi, "autoa"],
  [/\bautoa\b/gi, "kotxea"],
  [/\bkotxe bat\b/gi, "auto bat"],
  [/\bauto bat\b/gi, "kotxe bat"],
  [/\bumea\b/gi, "haurra"],
  [/\bhaurra\b/gi, "umea"],
  [/\bume bat\b/gi, "haur bat"],
  [/\bhaur bat\b/gi, "ume bat"],
  [/\bmutila\b/gi, "mutikoa"],
  [/\bmutikoa\b/gi, "mutila"],
  [/\bmutil bat\b/gi, "mutiko bat"],
  [/\bmutiko bat\b/gi, "mutil bat"],
  [/\bsemealabak\b/gi, "semealbak"],
  [/\bsemealbak\b/gi, "semealabak"]
];
...
function getPredicateNominalVariants(phrase) {
  ...
  const rawVariants = new Set([phrase.trim()]);

  // Direct table lookup
  if (PRED_SG_DEF_TO_INDEF.has(p)) {
    rawVariants.add(PRED_SG_DEF_TO_INDEF.get(p));
  }
  if (PRED_SG_INDEF_TO_DEF.has(p)) {
    rawVariants.add(PRED_SG_INDEF_TO_DEF.get(p));
  }
  if (PRED_PL_DEF_TO_INDEF.has(p)) {
    rawVariants.add(PRED_PL_DEF_TO_INDEF.get(p));
  }
  if (PRED_PL_INDEF_TO_DEF.has(p)) {
    rawVariants.add(PRED_PL_INDEF_TO_DEF.get(p));
  }

  // Multi-word phrase ending in "bat" (indefinite singular -> definite singular)
  ...
  if (batMatch) {
    ...
    rawVariants.add(defFull);
    rawVariants.add(head);
  }

  // Multi-word phrase ending in "batzuk" (indefinite plural -> definite plural)
  ...
  if (batzukMatch) {
    ...
    rawVariants.add(plDefFull);
  }

  // Definite noun / adjective phrase (singular or plural -> indefinite)
  if (!batMatch && !batzukMatch) {
    ...
    if (PRED_SG_DEF_TO_INDEF.has(lastWord)) {
      ...
      rawVariants.add(beforeLast ? `${beforeLast} ${indefLast}` : indefLast);
    } else {
      let indefLast = "";
    ...
    if (PRED_PL_DEF_TO_INDEF.has(lastWord)) {
      ...
      rawVariants.add(beforeLast ? `${beforeLast} ${indefPlLast}` : indefPlLast);
  ...
  const finalVariants = new Set(rawVariants);
  for (const v of rawVariants) {
    for (const [re, repl] of SYNONYMS) {
      if (re.test(v)) {
        finalVariants.add(v.replace(re, repl));
      }
    }
  }
  ...
  if (copulaIdx !== -1) {
    ...
    if (!isNegative) {
      ...
      if (copulaIdx === cleanWords.length - 1 && cleanWords.length >= 2) {
          ...
          if (predStr) {
            ...
            predVariants.forEach(pv => {
              if (candidateSubj) {
                ...
                } else if (PROPER_NOUNS.has(cleanWord(candidateSubj))) {
                  variations.add(`${pv} ${copula}`);
                  variations.add(`hura ${pv} ${copula}`);
                  variations.add(`${pv} ${copula} hura`);
                }
              } else {
                variations.add(`${pv} ${copula}`);
                if (impliedPronoun) {
        ...
        // Case B: Copula in the middle (e.g. "Nire lehengusua da ere camareroa", "Euskara da Euskal Herriko hizkuntza")
        const beforeCopula = words.slice(0, copulaIdx);
        const afterCopula = words.slice(copulaIdx + 1);

        const subjStr = beforeCopula.join(" ");
        const hasEre = afterCopula.some(w => cleanWord(w) === "ere");
        const predStr = hasEre ? afterCopula.filter(w => cleanWord(w) !== "ere").join(" ") : afterCopula.join(" ");
          ...
          predVariants.forEach(pv => {
            if (subjBefore) {
              ...
              if (impliedPronoun) {
                ...
                variations.add(`${impliedPronoun} ${pv} ez ${aux}`);
              }
            }
          });
        }
...
const UPGRADE_ONE_TO_A_CHALLENGE_IDS = new Set([
  56, 73, 89, 693, 696, 699, 703, 710, 727, 731, 736, 1501, 1503, 1507, 1599, 7343, 7376
]);

// Normalize English Prompts: replace "one [Noun]" when used purely as indefinite article
const NORMALIZE_PROMPT_MAP = {
  71: "You are Aitor. You are a student.",
  79: "You are Aitor. You are a student.",
  713: "You were a thin dog."
};
...
const processedChallenges = challenges.map(c => {
  ...
  // 1. English Prompt Normalization
  if (NORMALIZE_PROMPT_MAP[c.id]) {
    updated.prompt = NORMALIZE_PROMPT_MAP[c.id];
  }
  ...
  if (c.type === 'TRANSLATE') {
    ...
    if (UPGRADE_ONE_TO_A_CHALLENGE_IDS.has(c.id) && Array.isArray(c.options)) {
      updated.options = c.options.map(opt => {
        if (opt.text.toLowerCase() === 'one' || (opt.order === 1 && opt.text.toLowerCase() === 'a')) {
          const nextOpt = c.options.find(o => o.correct && o.order === opt.order + 1);
          const isVowelNext = nextOpt && /^[aeiou]/i.test(nextOpt.text);
          const isStartOfSentence = opt.order === 1;
          const isCapital = isStartOfSentence || /^[A-Z]/.test(opt.text);
          const art = isVowelNext ? (isCapital ? 'An' : 'an') : (isCapital ? 'A' : 'a');
          return { ...opt, text: art };
        }
        return opt;
      });
    ...
    let targetText = correctOptions.map(o => o.text).join(' ').trim();
    if (targetText && /^[a-z]/.test(targetText)) {
      targetText = targetText.charAt(0).toUpperCase() + targetText.slice(1);
    }
    ...
    if (isBasque) {
      if (targetText) {
        const generated = generateBasqueVariations(targetText, updated.prompt || '');
        generated.forEach(a => newAnswers.add(a));
      }
    } else {
      const generated = generateNaturalEnglishVariations(targetText, updated.prompt || '');
      generated.forEach(a => newAnswers.add(a));
      
      // Also generate variations on previous acceptedAnswers
      existingAnswers.forEach(ea => {
        const fromExisting = generateNaturalEnglishVariations(ea, updated.prompt || '');
    ...
    if (correctOpt && correctOpt.text) {
      ...
      if (!c.question.toLowerCase().includes('basque')) {
        const enVars = generateNaturalEnglishVariations(correctOpt.text.trim(), updated.prompt || '');
...
// Write to data/courses/1/challenges.json
