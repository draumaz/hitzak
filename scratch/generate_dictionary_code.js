...
// 2. Add explicit dictionary map for missing words gathered from inspection
...
// 5. Test complete audit of all prompt words across all 6658 challenges
const unmappedPromptWords = new Set();
let totalPromptsAudited = 0;
let totalWordsAudited = 0;
let totalWordsWithTooltips = 0;

for (const c of challenges) {
  if (!c.prompt) continue;
  totalPromptsAudited++;

  const words = c.prompt.split(/\s+/).map(w => {
    return w.replace(/^([.,\/#!$%\^&\*;:{}=\-_`~()?\"'\u201c\u201d\u2018\u2019\u00ab\u00bb\u2022\u2013\u2014()\[\]{}]*)(.*?)([.,\/#!$%\^&\*;:{}=\-_`~()?\"'\u201c\u201d\u2018\u2019\u00ab\u00bb\u2022\u2013\u2014()\[\]{}]*)$/, "$2");
  }).filter(Boolean);

  for (const w of words) {
    const lookup = cleanPunct(w);
    if (lookup === "audio" || lookup === "playback" || lookup === "review" || !lookup) continue;
    totalWordsAudited++;

    let hasTooltip = false;
    if (fullDict[lookup]) {
      hasTooltip = true;
    } else if (lookup.endsWith("s") && fullDict[lookup.slice(0, -1)]) {
      hasTooltip = true;
    }

    if (hasTooltip) {
      totalWordsWithTooltips++;
    } else {
      unmappedPromptWords.add(lookup);
    }
  }
}

console.log("=== AUDIT RESULTS ===");
console.log("Prompts Audited:", totalPromptsAudited);
console.log("Total Prompt Words Audited:", totalWordsAudited);
console.log("Words with Hoverable Tooltips:", totalWordsWithTooltips);
console.log("Tooltip Coverage Percentage:", ((totalWordsWithTooltips / totalWordsAudited) * 100).toFixed(2) + "%");
console.log("Unmapped Prompt Words Count:", unmappedPromptWords.size);
console.log("Unmapped List:", Array.from(unmappedPromptWords));
