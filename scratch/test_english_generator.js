...
// Comprehensive English linguistic generator
function generateNaturalEnglishVariations(target, prompt = "") {
  ...
  function expandIndefiniteArticle(text) {
    ...
    if (oneMatches) {
      let replaced = text;
      // Replace instances where "one [noun]" is not a purely mathematical number like "one plus", "one o'clock", "one hundred", "one thousand", "one million", "one billion", "one thirty", "one euro", "one january", "thirty one", "twenty one"
      replaced = replaced.replace(/\b(one)\s+(teacher|student|boy|girl|woman|man|fat cat|thin dog|cat|dog|owl|chicken|bird|purse|bag|car|house|pen|bottle of water|season|doctor|nurse|cook|chef|driver|waiter|stranger|friend|police officer|tourist|animal|person|snake|turtle|pig|horse|cow|sheep|goat|wolf|fish|book|hotel|inn|shop|store|school|restaurant|bar|street|table|door|window|shirt|dress|skirt|coat|sweater|jacket|apple|banana|lemon|pear|strawberry|peach)\b/gi, (match, oneWord, noun) => {
    ...
    if (artMatches) {
      let replaced = text;
      replaced = replaced.replace(/\b(a|an)\s+(teacher|student|boy|girl|woman|man|fat cat|thin dog|cat|dog|owl|chicken|bird|purse|bag|car|house|pen|bottle of water|season|doctor|nurse|cook|chef|driver|waiter|stranger|friend|police officer|tourist|animal|person|snake|turtle|pig|horse|cow|sheep|goat|wolf|fish|book|hotel|inn|shop|store|school|restaurant|bar|street|table|door|window|shirt|dress|skirt|coat|sweater|jacket|apple|banana|lemon|pear|strawberry|peach)\b/gi, (match, art, noun) => {
  ...
  function expandAlsoToo(text) {
    ...
    // "[Subj] is/am/are/was/were also [Pred]" <-> "[Subj] is/am/are/was/were [Pred] too" / "as well"
    ...
    // "[Subj] is/am/are/was/were [Pred] too" <-> "[Subj] is/am/are/was/were also [Pred]"
    ...
    // "[Subj] also [Verb] [Obj]" <-> "[Subj] [Verb] [Obj] too"
  ...
  function expandPunctuation(text) {
    ...
    // If text has two capitalized sentences joined by space, e.g. "Hello I am Miren I am a teacher"
    // or "Hi I am also a teacher"
    const words = text.trim().split(/\s+/);
    
    // Check if greeting at start: e.g. ["Hi", "I", "am", "a", "teacher"]
  ...
  currentSet.forEach(s => {
    expandIndefiniteArticle(s).forEach(x => afterIndef.add(x));
  });
  ...
  currentSet.forEach(s => {
    expandAlsoToo(s).forEach(x => afterAlso.add(x));
  });
  ...
  currentSet.forEach(s => {
    expandGreetings(s).forEach(x => afterGreetings.add(x));
  });
  ...
  currentSet.forEach(s => {
    applyContractions(s).forEach(x => afterContractions.add(x));
  });
  ...
  currentSet.forEach(s => {
    expandPunctuation(s).forEach(x => finalSet.add(x));
  });
...
// Test cases
console.log("=== TEST 1: Hi I am also one teacher ===");
console.log(generateNaturalEnglishVariations("Hi I am also one teacher", "Epa. Ni ere irakasle bat naiz."));

console.log("\n=== TEST 2: You are Aitor You are one student ===");
console.log(generateNaturalEnglishVariations("You are Aitor You are one student", "Zu Aitor zara. Zu ikasle bat zara."));

console.log("\n=== TEST 3: I was one fat cat ===");
console.log(generateNaturalEnglishVariations("I was one fat cat", "Ni katu gizen bat nintzen."));

console.log("\n=== TEST 4: Miren is also a teacher ===");
console.log(generateNaturalEnglishVariations("Miren is also a teacher", "Miren ere irakaslea da."));

console.log("\n=== TEST 5: They do not have cars ===");
console.log(generateNaturalEnglishVariations("They do not have cars", "Haiek ez daukate kotxerik."));
