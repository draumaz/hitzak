...
function generateClauseVariations(clause, prompt = "") {
  ...
  // Check multi-sentence splitting
  ...
  // Check conjunction splitting (e.g. "baina", "baizik", "eta", "edo")
    ...
    if (!isNegative) {
      ...
      if (copulaIdx === cleanWords.length - 1 && cleanWords.length >= 2) {
        ...
        if (ereIdx !== -1 && ereIdx < copulaIdx) {
          ...
          // Standard copula: [Subject]? [al]? [Predicate] [copula]
          let candidateSubj = "";
...
const processedChallenges = challenges.map(c => {
  ...
  // Preserve existing answers first, then append newly generated answers
...
const sampleIds = [21, 23, 30, 31, 34, 36, 37, 39, 41, 43, 80, 100];
